...
**Status.** Library shipped (`mcp/lib/findings-index.js` + `test/findings-index.test.js`, 14 tests passing). Hashed-feature-vector lexical similarity over `(title, description, attack_class, cwe, endpoint, surface_id, surface_type, tech_stack, evidence_summary, proof_of_concept[truncated])`. No ML dependency; uses 256-slot SHA-256 feature hashing with token + 2-gram bagging and stop-word filtering. `cosineSimilarity` ranks priors by relevance. `indexFinding` upserts by `finding_id`; `queryFindingsForTarget` and `queryFindingsCrossTarget` return top-K matches with severity / attack_class filters. MCP tool wrappers (`bounty_index_finding`, `bounty_query_findings_index`) and auto-indexing wired into `bounty_record_finding` are pending. Brief-time prior injection lands once the MCP tools are exposed.
