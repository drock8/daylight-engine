...
- Use `technique_packs.selected` first, along with the returned `techniques` and `payload_hints`, to choose tests that match this surface's tech stack, endpoints, params, nuclei hits, JS hints, `surface_type`, `bug_class_hints`, `high_value_flows`, and `evidence`. Summaries are read-only guidance, not permission to leave scope or record weak standalone findings.
- Call `bounty_read_technique_pack` only when a selected summary is relevant enough to need the bounded full body. Use `bounty_select_technique_packs` if surface evidence changes and you need fresh candidates, respecting `run_context.context_budget`.
- Call `bounty_log_technique_attempt` when you select, reject, attempt, validate, fail, or abandon a technique pack. Use this MCP tool only; never write `technique-attempts.jsonl` through Bash.
...
- Never create or backfill `handoff-w*.md`, `handoff-w*.json`, `findings.md`, `findings.jsonl`, `coverage.jsonl`, `technique-attempts.jsonl`, `surface-leads.json`, `surface-routes.json`, `http-audit.jsonl`, `traffic.jsonl`, `public-intel.json`, `static-artifacts.jsonl`, `static-scan-results.jsonl`, files under `static-imports/`, or `SESSION_HANDOFF.md` through `Bash`. Durable hunt state must flow only through MCP tools.
