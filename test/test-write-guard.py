...
TESTS = [
    ...
    ("Write to MCP-owned static-imports artifact → block",
     ...
     2),
    ("Write to MCP-owned mobile-artifacts.jsonl → block",
     {"tool_input": {"file_path": f"{SESSION}/mobile-artifacts.jsonl", "content": "test"}},
     2),
    ("Write to MCP-owned mobile artifact → block",
     {"tool_input": {"file_path": f"{SESSION}/mobile-apps/MA-1.bin", "content": "test"}},
     2),
    ("Write to MCP-owned mobile trace → block",
     {"tool_input": {"file_path": f"{SESSION}/mobile-traces/session.pcapng", "content": "test"}},
     2),
    ("Write to MCP-owned mobile device log → block",
     {"tool_input": {"file_path": f"{SESSION}/device-logs/logcat.txt", "content": "test"}},
     2),
    ("Write to MCP-owned mobile app container → block",
     {"tool_input": {"file_path": f"{SESSION}/app-containers/app.sqlite", "content": "test"}},
     2),
    ...
    ("Bash > to MCP-owned static-imports artifact → block",
     ...
     2),
    ("Bash > to MCP-owned mobile static scan results → block",
     {"tool_input": {"command": f"echo data > {SESSION}/mobile-static-scan-results.jsonl"}},
     2),
    ("Bash > to MCP-owned mobile screenshot → block",
     {"tool_input": {"command": f"echo data > {SESSION}/screenshots/login.png"}},
     2),
